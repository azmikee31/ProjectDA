...
function SingleMovie() {
  ...
  const { isLoading, isError, movies: movie } = useSelector(state => state.getMoviesDetail);
  const { movies } = useSelector(state => state.getAllMovies);
  const RelatedMovies = movies ? movies?.filter((m) => m.category === movie?.category) : null;
        ...
        : isError ? (
          ...
          <>
            ...
            <div className="container mx-auto min-h-screen px-2 my-6">
              ...
              {RelatedMovies ? RelatedMovies.length > 0 && <div className="my-16">
